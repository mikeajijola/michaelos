...
export function CapabilityInfo({ capabilityId, params = {}, controlLabel }: { capabilityId: string; params?: Record<string, unknown>; controlLabel: string }) {
  ...
  return <><button className="cap-info-button" aria-label={`Show Action Key and CLI details for ${controlLabel}`} title="Show capability details" onFocus={() => selectElement({ ...metadata, focused: true })} onMouseEnter={() => selectElement(metadata)} onClick={event => { event.preventDefault(); event.stopPropagation(); setOpen(true); }}>ⓘ</button>{open && <div className="cap-info-backdrop" role="presentation" onMouseDown={() => setOpen(false)}><section className="cap-info-panel" role="dialog" aria-modal="true" aria-label={`Capability details for ${controlLabel}`} onMouseDown={event => event.stopPropagation()}><button className="cap-info-close" aria-label="Close capability details" onClick={() => setOpen(false)}>×</button><div className="eyebrow">Capability details</div><h2>{capabilityId}</h2><p>{capability.description}</p><dl><dt>Parameters</dt><dd><code>{JSON.stringify(params, null, 2)}</code></dd><dt>CLI</dt><dd><code>{resolveCli(capability, params)}</code></dd><dt>Action Keys</dt><dd><code>{resolveTemplate(capability.keyboard.template, params)}</code></dd><dt>Accessible label</dt><dd>{controlLabel}</dd><dt>Risk</dt><dd>{capability.risk}</dd></dl><div className="gateway-note"><b>Agent Gateway Keys</b>{AGENT_GATEWAY.map(keys => <code key={keys.join("+")}>{keys.join("+")}</code>)}<span>Then enter the Action Key above.</span></div></section></div>}</>;
}
