...
export default function NaviModelSpikePage() {
  ...
  const run = () => { if (running) return; setRunning(true); setEvents([]); const instance = new Worker(new URL("../../../navigator/navi-spike.worker.ts", import.meta.url)); worker.current = instance; instance.onmessage = event => { setEvents(current => [...current, event.data]); if (event.data.type === "complete" || event.data.type === "error") { setRunning(false); void fetch("/__spike_results", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ userAgent: navigator.userAgent, webgpu: "gpu" in navigator, crossOriginIsolated, finishedAt: new Date().toISOString(), ...event.data }) }).catch(() => undefined); } }; instance.postMessage({ type: "run" }); };
